...
sys_read(read_args_t *arg)
{
        ...
        if (err < 0) { // Not sure whether would run to here
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                curthr->kt_errno = -err;
                return -1;
        }
        
        void *allocated_buf = page_alloc();

        int nbytes_read = do_read(kern_args.fd, allocated_buf, kern_args.nbytes);

        if (nbytes_read < 0) {
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                curthr->kt_errno = -nbytes_read;
                page_free(allocated_buf);
                return -1;
        }

        err = copy_to_user(kern_args.buf, allocated_buf, nbytes_read);
        if (err < 0) { // Not sure whether would run to here
...
sys_write(write_args_t *arg)
{
        ...
        err = copy_from_user(allocated_buf, kern_args.buf, kern_args.nbytes);
        if (err < 0) {
                dbg(DBG_PRINT, "(GRADING3D)\n");
                curthr->kt_errno = -err;
                return -1;
        ...
        int nbytes_written = do_write(kern_args.fd, allocated_buf, kern_args.nbytes);

        if (nbytes_written < 0) {
                dbg(DBG_PRINT, "(GRADING3D 1)\n");
                curthr->kt_errno = -nbytes_written;
                page_free(allocated_buf);
                return -1;
        }
...
sys_getdents(getdents_args_t *arg)
{
        ...
        int err;
        err = copy_from_user(&kern_args, arg, sizeof(getdents_args_t));
        if (err < 0) { // Not sure whether whould this part run
                dbg(DBG_PRINT, "(GRADING3D)\n");
                curthr->kt_errno = -err;
                return -1;
        }
        ...
        dirent_t de;
        int i = 0; 
        int times = (int)kern_args.count/sizeof(dirent_t);
        int result = 0;

        while (i < times) {
                int ret_val;
                ret_val = do_getdent(kern_args.fd, &de);
                if (ret_val < 0) {
                        dbg(DBG_PRINT, "(GRADING3D)\n");
                ...
                dbg(DBG_PRINT, "(GRADING3B)\n");
                copy_to_user(kern_args.dirp + i, &de, ret_val);
                result += ret_val;
                i += 1;
        }
