...
do_brk(void *addr, void **ret)
{
        ...
        if ((size_t) addr > USER_MEM_HIGH){
                dbg(DBG_PRINT, "(GRADING3E)\n");
                return -ENOMEM;
        }

        size_t pre_brk = (size_t)curproc->p_brk;
        size_t brk_end = NULL;
        size_t pre_brk_end;
        ...
        if (ADDR_TO_PN(PAGE_ALIGN_UP(addr)) == ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_brk))){
                ...
                *ret = addr;
                ...
                dbg(DBG_PRINT, "(GRADING3B)\n");
                brk_end = ADDR_TO_PN(PAGE_ALIGN_UP(addr));
                pre_brk_end = ADDR_TO_PN(PAGE_ALIGN_UP(pre_brk));
                vmmap_remove(curproc->p_vmmap, brk_end, pre_brk_end - brk_end);
        
        } 
        else {
                size_t brk_first = ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_brk));
                size_t brk_end = ADDR_TO_PN(PAGE_ALIGN_UP(addr));
                
                if (!vmmap_is_range_empty(curproc->p_vmmap, brk_first, brk_end - brk_first)){
                        dbg(DBG_PRINT, "(GRADING3E)\n");
                        return -ENOMEM;
                }

                dbg(DBG_PRINT, "(GRADING3B)\n");
                vmarea_t *vm_area = vmmap_lookup(curproc->p_vmmap, ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_start_brk)));

                if (vm_area == NULL){
                        dbg(DBG_PRINT, "(GRADING3B)\n");
                        vmmap_map(curproc->p_vmmap, NULL, ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_start_brk)), brk_end - ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_start_brk)),
                        PROT_READ | PROT_WRITE, MAP_PRIVATE, 0, VMMAP_DIR_LOHI, &vm_area);
                } 
                else {
                        dbg(DBG_PRINT, "(GRADING3B)\n");
                        vm_area->vma_end = brk_end;
                }
